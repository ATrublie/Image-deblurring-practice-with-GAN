...
class deblur_model():
    ...
    def generator_model(self):
        ...
        with tf.variable_scope('g_model'):
            ...
            for i in range(n_downsampling):
                ...
                _out = tf.layers.conv2d(_out, filters=ngf * mult * 2, kernel_size=(3, 3), strides=(2, 2),
                                        padding='VALID')
                _out = tf.layers.batch_normalization(_out, training=self.training)
                _out = tf.nn.relu(features=_out)
            ...
            for i in range(n_downsampling):
                ...
                # _out = tf.pad(_out,[[0,0],[1,1],[1,1],[0,0]], mode="CONSTANT")
                _out = tf.layers.conv2d_transpose(_out, filters=int(ngf * mult / 2), kernel_size=(3, 3), strides=(2, 2),
                                                  padding='SAME')
    ...
    def discriminator_model(self):
        ...
        with tf.variable_scope('d_model'):
            ...
            self.real_B = tf.placeholder(tf.float32, shape=[None, input_size, input_size, 3],
                                         name='real_B')  # a placeholder for the real sharp image
            ...
            _out = tf.layers.conv2d(_out, filters=ndf, kernel_size=kernel_size, strides=(2, 2), name='conv0',
                                    padding='VALID')
            ...
            for n in range(1, n_layers):
                ...
                _out = tf.layers.conv2d(_out, filters=ndf * nf_mulf, kernel_size=kernel_size, strides=(2, 2),
                                        name='conv{}'.format(n), padding='VALID')
            ...
            _out = tf.pad(_out, [[0, 0], [padw, padw], [padw, padw], [0, 0]])
            _out = tf.layers.conv2d(_out, filters=ndf * nf_mulf, kernel_size=kernel_size, strides=(1, 1),
                                    name='conv{}'.format(n_layers), padding='VALID')
        ...
        with tf.variable_scope('d_model', reuse=True):
            ...
            for n in range(1, n_layers):
                ...
                _out = tf.pad(_out, [[0, 0], [padw, padw], [padw, padw], [0, 0]])
                _out = tf.layers.conv2d(_out, filters=ndf * nf_mulf, kernel_size=kernel_size, strides=(2, 2),
                                        name='conv{}'.format(n), padding='VALID', reuse=True)
            ...
            _out = tf.pad(_out, [[0, 0], [padw, padw], [padw, padw], [0, 0]])
            _out = tf.layers.conv2d(_out, filters=ndf * nf_mulf, kernel_size=kernel_size, strides=(1, 1),
                                    name='conv{}'.format(n_layers), padding='VALID', reuse=True)
            _out = tf.layers.batch_normalization(_out, training=self.training, name='norm{}'.format(n_layers),
                                                 reuse=True)
            _out = tf.nn.leaky_relu(features=_out, alpha=0.2)
    ...
    def init_loss(self):
        ...
        self.D_trainer = tf.train.AdamOptimizer(learning_rate=0.0001, beta1=0.5, beta2=0.999).minimize(self.d_loss,
                                                                                                       var_list=d_vars)
        with tf.control_dependencies(g_update_ops):
            self.G_trainer = tf.train.AdamOptimizer(learning_rate=0.0001, beta1=0.5, beta2=0.999).minimize(self.g_loss,
                                                                                                           var_list=g_vars)
                ...
                for index in range(int(blur.shape[0] / batch_size)):
                    ...
                    if train_critic:
                        ...
                        generated_images = sess.run(self.fake_B,
                                                    feed_dict={self.real_A: blur_batch, self.training: True})
                    ...
                    if (i + 1) % show_freq == 0:
                        print(
                            "{}/{} batch in {}/{} epochs, discriminator loss: {}, generator loss: {}".format(index + 1,
                                                                                                             int(
                                                                                                                 blur.shape[
                                                                                                                     0] / batch_size),
                                                                                                             epoch + 1,
